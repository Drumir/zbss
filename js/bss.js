...
function showIt() {         // Отображает таблицу тикетов
  ...
  for(var key in List) {
    ...
    if(filter.notClient != "" && filter.notClient.toUpperCase().indexOf(List[key].client.toUpperCase()) != -1) continue;  // Если filter.notClient не пуст и этот тикет не соответствует, пропускаем тикет
  ...
  for(var i = 0; i < data.rows.length;  i ++) {
    ...
    if(Tickets[tt.id] === undefined){       // Если такого тикета в списке еще нет, добавим
      ...
      if(newbornTT.name != undefined && newbornTT.zhostid != undefined && Tickets[tt.id].name === newbornTT.name){
        Tickets[tt.id].zhostid = newbornTT.zhostid;   // Дляэтого свежесозданного тикета уже определен zhostid
...
function checkPing(){
  ...
  for(var key in Tickets)     // Сформируем массив тикетов, у которых нужно проверить пинг
//    if(Tickets[key].zhostid != undefined)    // Проверим все тикеты с известным zhostid
    if(Tickets[key].zhostid != undefined && Tickets[key].watchPing == true) // Проверим только тикеты с установленным watchPing
